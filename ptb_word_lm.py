...
flags.DEFINE_string(
  "cell", "LSTM", 
  "The cell for the model. Possible options are: LSTM and DRUM.")
flags.DEFINE_string(
    "model", "small",
    "A type of model. Possible options are: small, medium, large.")
flags.DEFINE_string("data_path", "/home/darumen/Research/Models/simple-examples/data",
                    "Where the training/test data is stored.")
flags.DEFINE_string("save_path", "/home/darumen/Research/Models/output/PTB/",
                    "Model output directory.")
flags.DEFINE_bool("use_fp16", False,
                  "Train using 16-bit floats instead of 32bit floats")

FLAGS = flags.FLAGS
filename = "./output/character/" + FLAGS.model + FLAGS.cell + ".txt"
f = open(filename, 'w')
...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, input_):
    ...
    if FLAGS.cell == "GORU": 
      def goru_cell(): 
        return GORUCell(
	    size, capacity, FFT, comp
	  )
      attn_cell = goru_cell 
      if is_training and config.keep_prob < 1:
        def attn_cell():
          return tf.contrib.rnn.DropoutWrapper(
              goru_cell(), output_keep_prob=config.keep_prob)

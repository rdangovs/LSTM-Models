...
import numpy as np
from rotational_models import _rotation 
...
class HyperDRUMCell(tf.contrib.rnn.RNNCell):
  '''
  HyperLSTM, with Ortho Initialization,
  Layer Norm and Recurrent Dropout without Memory Loss.
  ...
  @property
  def state_size(self):
    return tf.contrib.rnn.LSTMStateTuple(self.hyper_num_units,
                                         self.num_units+self.hyper_num_units)
  ...
  def __call__(self, x, state, timestep = 0, scope=None):
    with tf.variable_scope(scope or type(self).__name__):
      c, total_h = state
      h = total_h[:, 0:self.num_units]
      hyper_state = tf.contrib.rnn.LSTMStateTuple(c,
                                                  total_h[:,self.num_units:])
      ...
      W_xh = tf.get_variable('W_xh',
        [x_size, 2 * num_units], initializer=w_init)
      W_hh = tf.get_variable('W_hh',
        [num_units, 2 * num_units], initializer=h_init)
      bias = tf.get_variable('bias_drum',
        [2 * num_units], initializer=tf.constant_initializer(0.0))
      ...
      ux = hyper_norm(ux, hyper_output, embedding_size, num_units, 'hyper_ux')
      rx = hyper_norm(rx, hyper_output, embedding_size, num_units, 'hyper_rx')
      
      # split Whh contributions
      uh, rh = tf.split(hh, 2, 1)
      uh = hyper_norm(uh, hyper_output, embedding_size, num_units, 'hyper_uh')
      rh = hyper_norm(rh, hyper_output, embedding_size, num_units, 'hyper_rh')
      
      # split bias
      ub, rb = tf.split(bias, 2, 0) # bias is to be broadcasted.
      ub = hyper_bias(ub, hyper_output, embedding_size, num_units, 'hyper_ub')
      rb = hyper_bias(rb, hyper_output, embedding_size, num_units, 'hyper_rb')
      
      # u = update_input, r = reset_gate
      u = ux + uh + ub
      r = rx + rh + rb

      if self.use_layer_norm:
        concat = tf.concat([u, r], 1)
        concat = layer_norm_all(concat, batch_size, 2, num_units, 'ln_all')
        u, r = tf.split(concat, 2, 1)

      x_mixed = _linear(x, self.num_units, True)

      U  = _rotation(x_mixed, r, batch_size, self.num_units)

      cz = relu(x_mixed + tf.reshape(tf.matmul(U, tf.reshape(h, 
                [batch_size, self.num_units, 1])), [batch_size, self.num_units]))     


      if self.use_recurrent_dropout:
        cz = tf.nn.dropout(cz, self.dropout_keep_prob)
      new_h = u * h + (1 - u) * cz

      hyper_c, hyper_h = hyper_new_state
      new_total_h = tf.concat([new_h, hyper_h], 1)

    return new_h, tf.contrib.rnn.LSTMStateTuple(hyper_c, new_total_h)
